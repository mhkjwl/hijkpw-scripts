...
V6_PROXY=""
IP=""
for _url in "https://api4.ipify.org" "https://api.ipify.org" "https://ip.sb"; do
    IP=$(curl -sL -4 --connect-timeout 8 "$_url" 2>/dev/null | tr -d '\r\n')
    [[ -n "$IP" ]] && break
done
if [[ -z "$IP" ]]; then
    IP=$(curl -sL -6 --connect-timeout 8 "https://api6.ipify.org" 2>/dev/null | tr -d '\r\n')
    [[ -n "$IP" ]] && V6_PROXY="https://gh.hijk.art/"
fi
...
getVersion() {
    ...
    NEW_VER="$(normalizeVersion "$(curl -sSL -A "curl" -H "Accept: application/vnd.github+json" "${TAG_URL}" --connect-timeout 15 | sed -n 's/.*"tag_name"[[:space:]]*:[[:space:]]*"\([^"]*\)".*/\1/p' | head -1)")"
...
getData() {
    ...
    if [[ "$XTLS" = "true" ]]; then
        echo ""
        colorEcho $BLUE " XTLS 流控：使用当前 Xray 支持的 Vision（旧版 direct/origin 已弃用）"
        ...
        if [[ -z "$answer" ]]; then
            ...
            2)
                ...
                do
                    ...
                    ip=`curl -sL "https://ip-api.com/json/${host}"`
...
installBBR() {
    ...
    if [[ "$PMT" = "yum" ]]; then
        ...
        $CMD_INSTALL --install-recommends linux-generic linux-image-generic 2>/dev/null || $CMD_INSTALL --install-recommends linux-generic
        command -v grub-set-default >/dev/null 2>&1 && grub-set-default 0
        echo "tcp_bbr" >> /etc/modules-load.d/modules.conf
        INSTALL_BBR=true
    fi
}

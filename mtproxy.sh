...
Download_secret(){
	...
	if command -v curl >/dev/null 2>&1; then
		curl -fsSL "https://core.telegram.org/getProxySecret" -o "${mtproxy_secret}"
	else
...
Download_multi(){
	...
	if command -v curl >/dev/null 2>&1; then
		curl -fsSL "https://core.telegram.org/getProxyConfig" -o "${mtproxy_multi}"
	else
...
Centos_yum(){
	...
	yum update
	if [[ $? = 0 ]]; then
		yum install -y openssl-devel zlib-devel git curl
	else
		yum install -y openssl-devel zlib-devel git curl
	fi
...
Debian_apt(){
	...
	apt-get update
	if [[ $? = 0 ]]; then
		apt-get install -y build-essential libssl-dev zlib1g-dev git curl
	else
		apt-get install -y build-essential libssl-dev zlib1g-dev git curl
	fi
}
